...
import { getRestMessage, getCountdownCue } from "./restMessages";
import { getExistingPushSubscription, subscribeToPush, scheduleRestNotification, cancelRestNotification } from "./push";
...
function parseRepsRange(repsStr) {
  const range = String(repsStr || "").match(/(\d+)\s*-\s*(\d+)/);
...
function WorkoutTab({ phase, setPhase, weekIdx, setWeekIdx, dayIdx, setDayIdx, workoutLogs, setExerciseLog, globalWeekNum, T, editMode, overrides, setOverride, elapsed, isTimeSaved, resetWorkoutTimer, resetDay }) {
  ...
  useEffect(() => {
    if (typeof Notification !== "undefined" && Notification.permission === "granted") {
      getExistingPushSubscription().then(setPushSubscription);
    }
  }, []);
  ...
  // ينظّف/يلغي أي إشعار مجدول للراحة القديمة قبل ما ينتقل ليوم/أسبوع ثاني.
  useEffect(() => {
    return () => {
      if (restTimerRef.current?.pushMessageId) cancelRestNotification(restTimerRef.current.pushMessageId);
      setRestTimer(null);
    };
  ...
  const startRestTimer = (matchName, displayName, setsCount, repsRangeStr, currentSet) => {
    const { min, max } = parseRepsRange(repsRangeStr);
    const msg = getRestMessage({ exerciseName: matchName, sets: setsCount, minReps: min, maxReps: max, currentSet });
    const nextSet = Math.min(currentSet + 1, setsCount);
    const endTime = Date.now() + msg.restSeconds * 1000;
    const subline = `${displayName} · الجولة ${nextSet} من ${setsCount}`;
    setRestTimer({
    ...
    if (pushSubscription) {
      scheduleRestNotification({
        ...
        delaySeconds: msg.restSeconds,
        title: T("⏱ انتهت الراحة", "⏱ Rest finished"),
        body: subline,
      }).then(messageId => {
        if (!messageId) return;
        setRestTimer(t => (t && t.endTime === endTime) ? { ...t, pushMessageId: messageId } : t);
      });
    }
  };
  ...
  return (
    <div style={{ padding: "16px 16px 8px" }}>
                  ...
                  <div key={si} style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 4 }}>
                    <input type="text" inputMode="numeric" placeholder="-" value={(log.reps && log.reps[si]) || ""}
                      onChange={e => {
                        ...
                        if (val) startRestTimer(ex.name, defaultName, numSets, repsVal, si + 1);
                      }}
      ...
      {restTimer && (() => {
        ...
        const countdownCue = !finished ? getCountdownCue(restTimer.remaining) : null;
        const dark = finished ? "#0d1a10" : null;
        return (
          <div style={{
            ...
            <div style={{
              background: finished ? "rgba(90,160,107,0.97)" : "rgba(20,21,23,0.97)",
              backdropFilter: "blur(12px)", border: `1px solid ${finished ? COLORS.green : COLORS.gold}`,
              borderRadius: 20, padding: "10px 12px 10px 10px", boxShadow: "0 8px 24px rgba(0,0,0,0.4)",
              display: "flex", alignItems: "center", gap: 12,
            }}>
              <Ring pct={(restTimer.total - restTimer.remaining) / restTimer.total} size={54} stroke={4.5} color={dark || COLORS.gold}>
                <span style={{
                  fontSize: finished ? 18 : 14, fontWeight: 900, fontFamily: "'Cairo', sans-serif",
                  fontVariantNumeric: "tabular-nums", color: dark || COLORS.text,
                }}>
                  {finished ? "✓" : formatMMSS(restTimer.remaining)}
                </span>
              </Ring>
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ fontSize: 10, fontWeight: 700, color: dark ? "rgba(13,26,16,0.7)" : COLORS.mutedDim }}>
                  {finished ? T("انتهت الراحة", "Rest finished") : restTimer.headline}
                </div>
                <div style={{
                  fontSize: 12.5, fontWeight: 700, color: dark || COLORS.text,
                ...
                {!finished && (
                  <div style={{
                    fontSize: 11, color: countdownCue ? COLORS.rust : COLORS.gold, fontWeight: countdownCue ? 800 : 500,
                    overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", marginTop: 1,
                  }}>
                    {countdownCue || restTimer.cue}
                  </div>
                )}
              </div>
              <button onClick={() => {
                if (restTimer.pushMessageId) cancelRestNotification(restTimer.pushMessageId);
                setRestTimer(null);
              }} style={{
                flexShrink: 0, width: 26, height: 26, borderRadius: "50%", border: "none", cursor: "pointer",
                background: "rgba(255,255,255,0.15)", color: dark || COLORS.text, fontSize: 13, lineHeight: 1,
              }}>✕</button>
            </div>
          </div>
        );
