...
// بالبرنامج (تختلف عن "معدة"/"ديدلفت" الفصحى) — لازم الاثنتين عشان
// المطابقة تشتغل صح.
const ABS_KEYWORDS = ["معدة", "معده", "بطن", "بلانك", "كرنش"];
const CALF_KEYWORDS = ["بطات", "سمانة"];
...
export function classifyExercise(input) {
  ...
  // الترتيب مقصود: abs/calf و isolation كلماتها أدق (تمييزية) وتُفحص قبل
  // heavy، لأن كلمات heavy زي "بار"/"بنش" عامة وتنطبق كجزء من أسماء
  // تمارين عزل أو معدة (مثال: "بنش مرتفع بايسبس"، "بنش منخفض معده").
  if (has(name, ABS_KEYWORDS)) return "abs";
  if (has(name, CALF_KEYWORDS)) return "calf";
...
const CUES = {
  ...
  calf: {
    first: ["راحة قصيرة، السمانة ترد بسرعة", "خذ نفسك وارجع"],
    middle: ["حافظ على الإحساس بالعضلة", "لا تتعجّل، ثبّت القدم"],
    last: ["أخيرة — اضغط لين النهاية", "خلصنا، اعصرها آخر مرة"],
  },
};
